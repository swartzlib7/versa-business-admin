...
2. **Column filter** — pick a column currently rendered on the table, then a second-stage control: picklist of that column’s values when the field has options, **or** a typed value. **Add** commits it as a criterion.
