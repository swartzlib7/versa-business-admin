...
| **Status** | 🔧 **0.7.164** aurora: photo-reference green fold + vertical rays, one at a time, first after 90s (Off by default). Awaiting Stephen QA |
...
Matches §1 on beta `:3200` through **0.7.164** local (aurora fold + rays, 90s first wait). **0.7.158** remains on origin/master. 0.7.159/0.7.160/0.7.161 aurora passes were each revised on Stephen feedback; 0.7.164 is one green fold-and-rays patch at a time, first after 90s, Off by default. WU-12 is task **#289** — agent-verified, awaiting Stephen QA. WU-07 / WU-08 / WU-09 accepted 2026-09-05. Comet palettes (WU-11) accepted 2026-09-07.
