# Versa - Business Admin — remaining work (high level)
...
| **Sample data** | Done (0.7.133–0.7.142) | Settings → Modes Insert/Delete Sample Data; rows tagged `ba_sample:` including demo member users. Install accounts are Administrator (human) + COA (agent). Demo never swaps the live backend. Packaged DB ships empty except those two admins + Primary Org + catalog. |
...
| **Packaging / install** | Agent path done | Skill `business_admin` + manual §2.0 (clone, read manual, install, implement GET /api). Container/systemd still TBD. Versa AGi setup-enable prompt is the remaining host-installer slice. |
