...
export function listInstances(filters: {
  ...
  if (filters.type_api_name) {
    const want = foldRecordTypeApiName(filters.type_api_name);
    rows = rows.filter((r) => foldRecordTypeApiName(r.type_api_name) === want);
  }
...
export function updateInstance(id: string, input: UpdateInstanceInput): UpdateInstanceResult {
  ...
  const now = new Date().toISOString();
  const merged = input.data !== undefined ? input.data : cur.data;
  const next: RecordInstance = {
    ...
    data: {
      ...merged,
      id: cur.id,
      created_at: cur.data.created_at || cur.created_at,
      updated_at: now,
    },
    lines,
  };
