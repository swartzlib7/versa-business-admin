...
import {
  PAGE_BUILDER_GLOSSARY_ENTRIES,
  PAGE_BUILDER_GLOSSARY_SECTION,
  withGlossaryAudit,
  type GlossaryAudit,
...
export type GlossarySection = {
  ...
  name: string;
  description: string;
  /** GL-2 (Stephen, 2026-09-14 02:48): disabled sections hide from the book view. Default on. */
  enabled?: boolean;
} & Partial<GlossaryAudit>;
...
export type GlossaryEntry = {
  ...
  name: string;
  definition: string;
  /** GL-2: disabled entries hide from the book view. Default on. */
  enabled?: boolean;
} & Partial<GlossaryAudit>;
...
export const INITIAL_ENTRIES: GlossaryEntry[] = [
  ...
  ...PAGE_BUILDER_GLOSSARY_ENTRIES.map((row) => ({
    id: row.id,
    sectionId: PAGE_BUILDER_GLOSSARY_SECTION.id,
    name: row.name,
