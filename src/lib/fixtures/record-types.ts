...
export const recordTypes: RecordTypeDefinition[] = [
  ...
  },
  {
    ...
    description: 'Header = type of support available. Lines = support request tickets of that type (Statistics-style header + lines).',
    parent_kind: 'faculty',
    parent_api_name: 'communications',
    structure: 'header_lines',
    show_as_tab: true,
  ...
  },
  {
    ...
    label: 'Custom',
    description:
      'Parent for Environment Custom listings: Statistics, Cycle Strip, and Pages. Elements and Rendering Drivers live on Page Builder → Canvas.',
  ...
  },
  {
    ...
    active: true,
    is_system: true,
    object_api_name: 'page',
  },
  {
    id: 'rt-page_element',
    api_name: 'page_element',
    label: 'Elements',
    description:
      'Legacy Palette dump. Not a staff Custom tab. Type remains under Custom.',
    parent_kind: 'environment',
    parent_api_name: 'custom',
    structure: 'list',
    show_as_tab: false,
    ...
    active: true,
    is_system: true,
    object_api_name: 'page_element',
  },
  {
    id: 'rt-cycle_strip',
    api_name: 'cycle_strip',
    label: 'Cycle Strip',
    description:
      'Retired. The six homepage steps live on Page Builder → Branding → Cycle Strip.',
    parent_kind: 'environment',
    parent_api_name: 'custom',
    structure: 'list',
    show_as_tab: false,
    ...
    active: false,
    is_system: true,
    object_api_name: 'cycle_strip',
  },
  {
    id: 'rt-render_driver',
    api_name: 'render_driver',
    label: 'Rendering Drivers',
    description:
      'Catalog of Page Builder code renderers. Recipes live in product code, not on this record. Listed on Page Builder → Canvas → Rendering Drivers, not Environment → Custom.',
    parent_kind: 'environment',
    parent_api_name: 'custom',
    structure: 'list',
    show_as_tab: false,
    ...
    active: true,
    is_system: true,
    object_api_name: 'render_driver',
  },
  {
    id: 'rt-driver_pairing',
    api_name: 'driver_pairing',
    label: 'Configuration',
    description: 'Instance on a Cell: this record × this driver recipe. Listed on Page Builder → Canvas → Elements, not Environment → Custom.',
    parent_kind: 'environment',
    parent_api_name: 'custom',
    structure: 'list',
    show_as_tab: false,
    ...
    active: true,
    is_system: true,
    object_api_name: 'driver_pairing',
  },
  {
  ...
  },
  {
    ...
    label: 'Transactions',
    description: 'Treasury documents: transaction, quote, estimate, or invoice (header + line items).',
    parent_kind: 'faculty',
  ...
  },
  {
    ...
    api_name: 'statistics',
    label: 'Statistics',
    description: 'Statistics as header + captured lines (I5.6.35 S-4). Each header defines a scale axis and a frequency window and renders as a graph; lines are the captured data points (UI + API capture share one validation layer).',
    parent_kind: 'environment',
    parent_api_name: 'custom',
    structure: 'header_lines',
    show_as_tab: true,
...
export function applyRecordTypeOverlay(overlay: {
  ...
  const over = (overlay.recordTypes ?? []).map((row) => ({
    ...row,
    api_name: foldRecordTypeApiName(row.api_name),
    object_api_name: foldRecordTypeApiName(row.object_api_name || row.api_name),
  }));
...
export function getRecordType(apiName: string): RecordTypeDefinition | undefined {
  ensureRecordTypesHydrated();
  const name = foldRecordTypeApiName(apiName);
  return recordTypeLive().find((r) => r.api_name === name);
}
...
export function listEditorParents(): EditorParent[] {
  return [
    ...
    { parent_kind: "environment", parent_api_name: "custom", label: "Custom", group: "Environment", baked_in_tabs: [] },
