/** Client side of the proof-of-work challenge issued by GET /api/auth/challenge. */
...
/** Fetch a fresh challenge and solve it. */
export async function solvedChallenge(): Promise<Challenge & { solution: number }> {
  const json = (await fetch("/api/auth/challenge").then((r) => r.json())) as { data?: Challenge };
  if (!json.data) throw new Error("No challenge");
  const solution = await solveChallenge(json.data);
