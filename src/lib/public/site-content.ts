...
export function ensureEnvironmentStatsSeeded(): void {
  const existing = listInstances({ type_api_name: "statistics" });
...
export async function listPublicStats(): Promise<PublicStat[]> {
  const rows = listInstances({
    type_api_name: "statistics",
    parent_kind: "environment",
  });
  // I5.6.35 S-6: headers are now header + lines (scale/frequency config in
  // the header JSONB; captured points in stat_line via the capture API). The
  // public card keeps its legacy shape: value = latest captured line (falls
  // back to the legacy flat value), series = captured line values in slot
  // order (falls back to the legacy series JSON / generated series).
  const lineMaps = await Promise.all(
    rows.map(async (row) => {
      const isHeaderModel = row.data.scale_end !== undefined || row.data.frequency_type !== undefined;
      if (!isHeaderModel || !isPostgresDataSource()) return null;
      try {
        return await listStatLinesDb(row.id);
      } catch {
        return null;
      }
    }),
  ...
  return rows.map((row, idx) => {
    const lines = lineMaps[idx];
    let value = row.data.value || "0";
    let series: number[] | null = null;
    if (lines && lines.length) {
      const captured = lines.filter((ln) => String(ln.value ?? "").trim() !== "");
      if (captured.length) {
        const last = captured[captured.length - 1];
        value = last.value;
        series = captured.map((ln) => Number(ln.value));
      } else {
        value = "0";
        series = [];
      }
    ...
    return {
      id: row.id,
      label: row.name,
      value: value || String(valueNum),
      unit: row.data.unit || "",
      category: row.data.category || "Environment",
      scale,
      series: series ?? parseSeries(row.data.series, valueNum, scale),
    };
  });
}
