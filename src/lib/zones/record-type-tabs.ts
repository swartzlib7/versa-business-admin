...
function fieldsFromCatalog(objectApiName: string) {
  const defs = listFieldDefinitions(objectApiName).filter((f) =>
    objectApiName === "statistics" ? isLiveStatField(f.api_name) : true,
  );
  if (!defs.length) {
    return [
...
function wireBakedChildToSystemType(
  ...
  const systemType = recordTypes.find((t) => {
    if (t.api_name !== apiName || !t.active) return false;
    if (t.parent_kind === parentKind && t.parent_api_name === tabId) return true;
    return false;
  });
...
export function applyRecordTypesToTab(
  ...
  const types = recordTypes.filter(
    (t) =>
      ...
      !(tab.id === "custom" && CUSTOM_INFRA_TYPES.has(t.api_name)),
  );
