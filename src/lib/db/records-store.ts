...
import {
  listStatLinesForHeaders,
  restampStatLinesForHeader,
  seedStatLinesForHeader,
} from '@/lib/db/stat-lines-store';
import type { StatLineRow } from '@/lib/db/stat-lines-store';
...
export async function ensureRecordTypesFromRegistry(): Promise<void> {
  ...
  for (const t of recordTypes) {
    ...
    const cur = byApiName.get(t.api_name) ?? byId.get(t.id);
    if (!cur) {
      await db
        .insert(recordTypeTable)
        .values({
          id: t.id,
          orgId,
          apiName: t.api_name,
...
function statLineToRecordLine(ln: StatLineRow): RecordInstanceLine {
  return {
    id: ln.id,
    record_id: ln.header_id,
    data: {
      line_slot: String(ln.slot),
      line_value: ln.value,
      line_stamp: ln.stamp ?? '',
      series: String(ln.series),
    },
    sort_order: ln.series * 1000 + ln.slot,
  };
}

async function loadLinesForRecords(
  ids: string[],
  typeById: Map<string, string>,
): Promise<Map<string, RecordInstanceLine[]>> {
  const generic = await loadLines(ids);
  const statIds = ids.filter((id) => typeById.get(id) === 'statistics');
  if (!statIds.length) return generic;
  const byHeader = await listStatLinesForHeaders(statIds);
  for (const id of statIds) {
    generic.set((id), (byHeader.get(id) ?? []).map(statLineToRecordLine));
  }
  return generic;
}
...
function toInstance(
  ...
  return {
    ...
    data: {
      ...header,
      id: header.id || row.id,
      created_at: header.created_at || createdAt,
      updated_at: header.updated_at || updatedAt,
      ...(row.createdBy && !header.created_by ? { created_by: row.createdBy } : {}),
    },
...
export async function listRecordsDb(
  ...
  if (filters.type_api_name)
    conds.push(eq(recordTypeTable.apiName, foldRecordTypeApiName(filters.type_api_name)));
  ...
  const typeById = new Map(rows.map((r) => [r.record.id, r.type.apiName]));
  const linesMap = await loadLinesForRecords(ids, typeById);
...
export async function getRecordDb(id: string): Promise<RecordInstance | null> {
  ...
  const typeById = new Map([[id, rows[0].type.apiName]]);
  if (rows[0].type.apiName === "statistics") {
    await seedStatLinesForHeader(id);
  }
  const linesMap = await loadLinesForRecords([id], typeById);
...
export async function createRecordDb(
  ...
  const typeRows = await db
    ...
    .where(eq(recordTypeTable.apiName, foldRecordTypeApiName(input.type_api_name.trim())))
    .limit(1);
  ...
  if (type.apiName === 'statistics') {
    await seedStatLinesForHeader(recordId);
  } else {
...
export async function updateRecordDb(
  ...
  if (input.lines !== undefined) {
    const typeRows = await db
      ...
      .where(eq(recordTypeTable.id, cur[0].recordTypeId))
      .limit(1);
    if (typeRows[0]?.apiName !== 'statistics') {
  ...
  const typeAfter = await db
    .select({ apiName: recordTypeTable.apiName })
    .from(recordTypeTable)
    .where(eq(recordTypeTable.id, cur[0].recordTypeId))
    .limit(1);
  if (typeAfter[0]?.apiName === 'statistics') {
    await restampStatLinesForHeader(id);
  }
