...
import { assertOrganizationFields, mergeOrganizationData } from "../organizations/field-validate";
...
export const postgresAdapter: DataAdapter = {
  ...
  async updateOrganization(id: string, input: UpdateOrganizationInput): Promise<Organization | null> {
    ...
    if (input.data) {
      assertOrganizationFields({ name: input.name ?? current.name, data: input.data });
      const data = mergeOrganizationData((row.data ?? {}) as Record<string, unknown>, input.data);
