...
//   2. Hosting: production = AWS Lightsail (app + Postgres). Review = local Postgres.
...
export const siteSettings = pgTable(
  'site_settings',
  {
    ...
    constellationVariant: text('constellation_variant', { enum: ['classic', 'realistic'] }).notNull().default('realistic'),
...
// Statistics -> Environment (S-4, 2026-09-12): captured data points for a
// statistic header. One header (a `record` row of type environment_stat) owns
// many lines. `series` is the series number within the header (Dynamic Series
// closes a full series and continues fresh from the next frequency slot;
// Single Series holds exactly one). `slot` is the frequency position
// (0-based) within the series. `source` records the capture path
// (ui | api | script). Indexed per the locked contract (header_id, series,
// slot) for window queries and capacity checks.
export const statLine = pgTable(
  'stat_line',
  {
    id: text('id').primaryKey().default(sql`gen_random_uuid()::text`),
    headerId: text('header_id')
      .notNull()
      .references(() => record.id, { onDelete: 'cascade' }),
  ...
  (table) => [
    index('stat_line_header_series_slot_idx').on(
      table.headerId,
      table.series,
      table.slot,
    ),
    index('stat_line_header_idx').on(table.headerId),
    uniqueIndex('stat_line_header_series_slot_unique').on(
      table.headerId,
      table.series,
      table.slot,
    ),
    uniqueIndex('stat_line_header_stamp_unique').on(table.headerId, table.stamp),
  ],
);
