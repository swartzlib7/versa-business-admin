    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1 max-w-3xl">
          Customize all zone entities including baked-in tabs. Add fields and named record types
          (list / header / header+lines). Manage picklists for dynamic types and baked-ins.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <ul className="space-y-1">
              ...
              {typesForParent.length === 0 && <li className="text-sm text-muted-foreground">No types yet.</li>}
            </ul>
          </CardContent>
        </Card>
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Named tab under the selected parent.</CardDescription>
          </CardHeader>
      ...
      {selectedType && (
        <Card>
          <CardHeader><CardTitle className="text-base">Fields - {selectedType}</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
              ...
              <select className="rounded-md border border-input bg-background px-3 py-2 text-sm" value={newField.data_type} onChange={(e) => setNewField((s) => ({ ...s, data_type: e.target.value }))}>
                {["text", "long_text", "number", "picklist", "boolean", "date"].map((d) => <option key={d} value={d}>{d}</option>)}
              </select>
              <select className="rounded-md border border-input bg-background px-3 py-2 text-sm" value={newField.value_set_api_name} onChange={(e) => setNewField((s) => ({ ...s, value_set_api_name: e.target.value }))}>
                <option value="">(no value set)</option>
                {valueSets.map((v) => <option key={v.api_name} value={v.api_name}>{v.label}</option>)}
              </select>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Options for dynamic types and baked-in entities.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex flex-wrap gap-2">{valueSets.map((v) => <Badge key={v.api_name} variant="secondary">{v.api_name}</Badge>)}</div>
          <div className="grid gap-3 sm:grid-cols-4">
            <Input placeholder="picklist_api" value={newVs.api_name} onChange={(e) => setNewVs((s) => ({ ...s, api_name: e.target.value }))} />
            <Input placeholder="Label" value={newVs.label} onChange={(e) => setNewVs((s) => ({ ...s, label: e.target.value }))} />
            <Input placeholder="First option" value={newVs.option} onChange={(e) => setNewVs((s) => ({ ...s, option: e.target.value }))} />
          ...
          <div className="grid gap-3 sm:grid-cols-4">
            <select className="rounded-md border border-input bg-background px-3 py-2 text-sm" value={addOpt.vs} onChange={(e) => setAddOpt((s) => ({ ...s, vs: e.target.value }))}>
              ...
              {valueSets.map((v) => <option key={v.api_name} value={v.api_name}>{v.api_name}</option>)}
            </select>
            <Input placeholder="option_value" value={addOpt.api_value} onChange={(e) => setAddOpt((s) => ({ ...s, api_value: e.target.value }))} />
            <Input placeholder="Option label" value={addOpt.label} onChange={(e) => setAddOpt((s) => ({ ...s, label: e.target.value }))} />
