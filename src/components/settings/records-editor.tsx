...
const EDITOR_TABS = ["types", "fields", "picklists", "layouts"] as const;
type EditorTab = (typeof EDITOR_TABS)[number];

function isEditorTab(value: string | null): value is EditorTab {
  return !!value && (EDITOR_TABS as readonly string[]).includes(value);
}

function recordsEditorHref(opts: { tab?: EditorTab; type?: string; parent?: string }): string {
  const params = new URLSearchParams();
  if (opts.parent) params.set("parent", opts.parent);
  if (opts.tab && opts.tab !== "types") params.set("tab", opts.tab);
  if (opts.type) params.set("type", opts.type);
  const q = params.toString();
  return q ? "/records-editor?" + q : "/records-editor";
}
...
export function RecordsEditor() {
  ...
  const section: EditorTab = isEditorTab(urlTab) ? urlTab : "types";
  const layoutFocusType = searchParams.get("type") || "";
  ...
  const [vsKindFilter, setVsKindFilter] = useState<"" | "standard" | "custom">("");
  const [vsFilter, setVsFilter] = useState("");
  const [layoutParentFilter, setLayoutParentFilter] = useState("");
  const [layoutKindFilter, setLayoutKindFilter] = useState<"" | "standard" | "custom">("");
  const [layoutStructureFilter, setLayoutStructureFilter] = useState("");
    ...
    fetch('/api/records/' + encodeURIComponent(recordId), { credentials: 'include' })
      ...
      .then((json) => {
        ...
        if (inst.type_api_name) {
          router.replace(recordsEditorHref({ tab: "types" }));
  ...
  const filteredValueSets = useMemo(() => {
    let rows = [...valueSets];
    if (vsKindFilter === "standard") rows = rows.filter((vs) => !!vs.is_system);
    if (vsKindFilter === "custom") rows = rows.filter((vs) => !vs.is_system);
    if (vsFilter) {
      const q = vsFilter.toLowerCase();
      rows = rows.filter(
        (vs) =>
          vs.api_name.toLowerCase().includes(q) ||
          vs.label.toLowerCase().includes(q),
      );
    }
    const get = (vs: VS) => {
      switch (vsSort.key) {
    ...
    return sortByText(rows, vsSort.dir, get);
  }, [valueSets, vsSort, vsItems, vsKindFilter, vsFilter]);
  ...
  return (
    <div className="space-y-3">
      <PageHeader
        ...
        tabsValue={section}
        onTabChange={(id) => {
          const next = isEditorTab(id) ? id : "types";
          setSubTab("configuration");
          router.replace(
            recordsEditorHref({
              tab: next,
              type: next === "layouts" ? layoutFocusType || undefined : undefined,
                              ...
                              <div className="border-t border-border bg-muted/20 px-4 py-4 sm:px-6" style={{ boxShadow: `inset 3px 0 0 ${theme.colors.brand}` }}>
                                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                  ...
                                  <div className="space-y-1 text-sm sm:col-span-2 lg:col-span-3">
                                    ...
                                    <p>
                                      ...
                                      >
                                        Open {t.label || t.api_name} layout
                                      </Link>
                                    </p>
                                  </div>
                                </div>
                                <div className="mt-4 flex gap-2">
                                  ...
                                  <Button
                                    ...
                                    disabled={busy}
                                    onClick={() => {
                                      setSelectedTypeForFields(t.api_name);
                                      router.replace(
                                        recordsEditorHref({
                                          tab: "fields",
                                          parent: parentFilter || undefined,
        ...
        <div role="tabpanel" className="space-y-3">
          ...
          <Card className="min-h-[640px] overflow-hidden">
            <CardHeader className="border-b bg-muted/30">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="mt-1 text-sm text-muted-foreground">Value sets for picklist fields. Filter by kind or search. Expand a row to view, add, or delete options (delete prompts for replacement when referenced).</p>
                </div>
                <div className="flex shrink-0 flex-col items-end gap-1">
                  <div className="flex flex-nowrap items-center justify-end gap-2">
                    <select
                      className="rounded-md border border-border bg-background px-3 py-1.5 text-sm"
                      value={vsKindFilter}
                      onChange={(e) => setVsKindFilter((e.target.value || "") as "" | "standard" | "custom")}
                    >
                      <option value="">All kinds</option>
                      <option value="standard">Standard / DB Core</option>
                      <option value="custom">Custom</option>
                ...
                <table className="w-full min-w-[500px] border-collapse text-left text-sm">
                  ...
                  <tbody>
                    ...
                    {filteredValueSets.length === 0 && (
                      <tr><td colSpan={picklistCols.length} className="px-4 py-8 text-center text-muted-foreground">{valueSets.length === 0 ? "No picklists yet. Use New Picklist to add the first one." : "No picklists match these filters."}</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
        ...
        <div role="tabpanel" className="space-y-3">
          ...
          <Card className="min-h-[640px] overflow-hidden">
            <CardHeader className="border-b bg-muted/30">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="mt-1 text-sm text-muted-foreground">Visual form layout builder. Filter by parent, kind, or structure. Configure sections, field order, visibility, and 1 / 2 / 4 / 6 / 8 column grids.</p>
                </div>
                <div className="flex shrink-0 flex-col items-end gap-1">
                  <div className="flex flex-nowrap items-center justify-end gap-2">
                    <select
                      className="rounded-md border border-border bg-background px-3 py-1.5 text-sm"
                      value={layoutParentFilter}
                      onChange={(e) => setLayoutParentFilter(e.target.value)}
                    >
                      <option value="">All parents</option>
                      {parents.map((p) => (
                        <option key={p.parent_kind + ":" + p.parent_api_name} value={p.parent_kind + ":" + p.parent_api_name}>{p.group ? p.group + " — " : ""}{p.label}</option>
                    ...
                    <select
                      className="rounded-md border border-border bg-background px-3 py-1.5 text-sm"
                      value={layoutKindFilter}
                      onChange={(e) => setLayoutKindFilter((e.target.value || "") as "" | "standard" | "custom")}
                    >
                      <option value="">All kinds</option>
                      <option value="standard">Standard / DB Core</option>
                      <option value="custom">Custom</option>
                    ...
                    <select
                      className="rounded-md border border-border bg-background px-3 py-1.5 text-sm"
                      value={layoutStructureFilter}
                      onChange={(e) => setLayoutStructureFilter(e.target.value)}
                    >
            ...
            <CardContent className="p-0">
              <LayoutEditorWrapper
                key={layoutFocusType || "layout-none"}
                parentFilter={layoutParentFilter}
                kindFilter={layoutKindFilter}
                structureFilter={layoutStructureFilter}
                search={layoutSearch}
                initialTypeApiName={layoutFocusType}
              />
            </CardContent>
          </Card>
        </div>
