...
const emptyDraft = (brand: ReturnType<typeof useBrand>): BrandDraft => ({
  ...
  zoom: clampSkyZoom(brand.constellation_zoom ?? SKY_STARS_ZOOM_DEFAULT),
...
export function BrandingPanel({
  ...
  useEffect(() => {
    ...
    const url = URL.createObjectURL(draft.musicFile);
    setMusicPreview(url);
    ...
    Promise.all([
      ...
      .then(([branding, publicContent]) => {
        ...
        setDraft({
          ...
          zoom: clampSkyZoom(b.constellation_zoom ?? SKY_STARS_ZOOM_DEFAULT),
  ...
  const handleSave = async () => {
    ...
    try {
      if (draft.musicFile) {
        ...
        form.append("file", draft.musicFile);
        const up = await fetch("/api/settings/branding/music", {
          method: "POST",
          body: form,
        });
        if (!up.ok) {
          const payload = await up.json().catch(() => null);
          setSaveError(payload?.error?.message ?? "Music upload failed.");
          return;
        }
      }
      const res = await fetch("/api/settings/branding", {
        ...
        body: JSON.stringify({
          ...
          brand_music_loop: draft.musicLoop,
          brand_music_autoplay: draft.musicAutoplay,
          brand_music_url: draft.musicRemoved && !draft.musicFile ? null : undefined,
          brand_name_in_menu: draft.nameInMenu,
        }),
      });
      if (!res.ok) {
        ...
        <div className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              />
              <BooleanSwitch
                checked={draft.nameInMenu}
                onChange={(nameInMenu) => patch({ nameInMenu })}
                label={draft.nameInMenu ? "Show name beside menu logo" : "Hide name beside menu logo"}
              />
              <p className="text-xs text-muted-foreground">
                Browser tab title always uses this name. The visitor header can hide the words and keep the logo.
              </p>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Color</label>
          ...
          <div className="space-y-3">
            ...
            <FileField
              ...
              removeLabel="Remove music"
              hint={MUSIC_UPLOAD_HINT}
              onFile={(file) => {
            ...
            <div className="flex flex-wrap gap-4">
              <BooleanSwitch
                ...
                onChange={(musicLoop) => patch({ musicLoop })}
                label={draft.musicLoop ? "Loop On" : "Loop Off"}
              />
              <BooleanSwitch
                checked={draft.musicAutoplay}
                onChange={(musicAutoplay) => patch({ musicAutoplay })}
                label={draft.musicAutoplay ? "Auto-Play On" : "Auto-Play Off"}
              />
            </div>
            <p className="text-xs text-muted-foreground">
              Auto-Play On starts the track as soon as the visitor page allows it. If the browser blocks sound, the first
              click anywhere on the page unmutes — the speaker is not required. Auto-Play Off stays paused until someone
              uses the speaker.
            </p>
            {!draft.musicRemoved && (draft.musicFile || draft.musicUrl) ? (
              <audio
                className="w-full"
                controls
                preload="metadata"
                src={musicPreview || draft.musicUrl || BRAND_MUSIC_HREF}
