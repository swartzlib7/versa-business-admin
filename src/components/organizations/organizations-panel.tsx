...
import { organizationFieldError } from '@/lib/organizations/field-validate';
...
export function PrimaryOrgPanel({
  accent,
  children,
}: {
  accent?: string;
  /** Optional content folded into this card, below a divider (#298). */
  children?: ReactNode;
}) {
...
function extraOrgData(draft: Record<string, string>): Record<string, unknown> {
  ...
  for (const [k, v] of Object.entries(draft)) {
    ...
    if (k === "is_active") data[k] = v === "" ? "" : v === "true";
    else data[k] = v;
  }
  return data;
}
...
function orgFields(
  ...
  return listingFieldsFromCatalog('organization').map((f) => ({
    ...
    required: f.required,
    lookupObjectApiName: f.lookupObjectApiName,
    readOnly: ORG_READONLY_KEYS.has(f.key),
...
function orgFormat(row: Organization, key: string, raw: string, orgs: Organization[]): string {
  ...
  if (key === 'logo_url' && raw) return raw.startsWith('data:image/') ? 'Image' : raw;
...
export function OrganizationsPanel({ accent }: { accent?: string }) {
  ...
  const onAdd = async (draft: Record<string, string>) => {
    const invalid = organizationFieldError(draft);
    if (invalid) {
      setNote(invalid);
      return false;
    }
