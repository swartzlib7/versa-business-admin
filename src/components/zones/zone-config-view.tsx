...
import {
  FloatTwinDock,
  SpatialTwinPane,
} from "@/components/zones/spatial-twin-pane";
import { TwinSlotProvider, useTwinSlot } from "@/components/zones/twin-slot-context";
import { twinFocusNodeId } from "@/lib/zones/spatial-twin";
...
// endpoint), not the generic record-lines PATCH path.
import {
  frequencyTypeLabel,
  openSeriesNumber,
...
function filterLiveCatalogFields<T extends { api_name: string }>(
  ...
  if (objectApiName !== "statistics") return fields;
  return fields.filter((f) => isLiveStatField(f.api_name));
}
...
function defaultChildIdForTab(tab: ZoneTab): string {
  if (tab.id === "org-configuration") return "org-organizations";
    ...
    return [
      {
        ...
        label: "Organizations",
        summary: "Organizations in this Versa - Business Admin.",
        fields: [],
        relations: [],
        sampleRows: undefined,
      },
      { ...tab, label: "Configuration", sampleRows: undefined },
    ];
  }
...
function ListingPanel({
  ...
  renderExpandedRow?: (recordId: string) => ReactNode;
  expandedEditing?: boolean;
  onRowEdit?: (recordId: string) => void;
  ...
  useEffect(() => {
    ...
    void fetch("/api/records/" + headerRecordIdProp, {
      credentials: "include",
      signal: controller.signal,
    })
      .then((r) => (r.ok ? r.json() : { data: null }))
      .then((json) => {
        const data = ((json.data as { data?: Record<string, unknown> } | null)?.data ??
          {}) as Record<string, unknown>;
        const parsed = parseStatHeaderConfig(data);
        setStatHeader(parsed.ok ? parsed.config : null);
      })
      .catch(() => setStatHeader(null));
    return () => controller.abort();
  }, [panel.recordTypeApiName, headerRecordIdProp, linesEpoch]);
  ...
  useEffect(() => {
    ...
    return () => controller.abort();
  }, [isDynamic, panel.recordTypeApiName, objectApiName]);
    ...
    if (isDynamic && catalogFields.length > 0) {
      ...
      if (isHeaderLines) {
        ...
        if (viewMode === "instances") {
          if (objectApiName === "statistics") {
            return catalogFields
              .filter((f) => f.api_name === "name")
              .map((f) => f.label);
          }
          return catalogFields
        ...
        if (objectApiName === "statistics") {
          return [frequencyTypeLabel(statHeader?.frequencyType), "Stamp", "Value"];
        }
        return catalogFields
  ...
  }, [isDynamic, catalogFields, panel.listColumns, panel.fields, panel.structure, viewMode, objectApiName, statHeader]);
  ...
  const fields: ListingField[] = useMemo(() => {
    if (isDynamic && catalogFields.length > 0) {
      ...
      const spanByApi = new Map<string, number>();
      for (const sec of runtime.edit) {
        for (const f of sec.fields) {
          if (f.span && f.span > 1) spanByApi.set(f.key, f.span);
        }
      }
      ...
      if (viewMode === "instances") {
        const headerFields = mapped.filter((f) => f.zoneRole == null || f.zoneRole === "header");
        if (objectApiName === "statistics") {
          return headerFields.map((f) =>
            f.key === "start_datetime"
              ? { ...f, required: true, defaultValue: toDatetimeLocalValue(new Date()) }
              : f,
          );
        }
        return headerFields;
      ...
      if (panel.structure === "header_lines") {
        const listFields = mapped.filter((f) => f.zoneRole == null || f.zoneRole === "list");
        if (objectApiName === "statistics") {
          const freqLabel = frequencyTypeLabel(statHeader?.frequencyType);
          const slotLabels = statHeader
            ? frequencySlotLabels(statHeader, statHeader.frequencyQty)
            : [];
          return listFields.map((f) => {
            if (f.key === "line_value") return { ...f, label: "Value", column: true };
            if (f.key === "line_slot") {
              return {
                ...f,
                label: freqLabel,
                ...
                options: slotLabels.map((_, i) => String(i)),
                optionLabels: slotLabels.length ? slotLabels : undefined,
              };
            }
            if (f.key === "line_stamp") {
              return {
  ...
  const [error, setError] = useState("");
  /** Slots that already have a captured value — empty seeds stay available. */
  const capturedSlots = useMemo(
    () =>
      rows
        .filter((r) => String(r.cells.line_value ?? "").trim() !== "")
        .map((r) => ({
          series: Number(r.cells.series || 1) || 1,
          slot: Number(r.cells.line_slot),
        }))
        .filter((r) => Number.isInteger(r.slot)),
    [rows],
  );
  const listingFields = useMemo(() => {
    ...
    const open = openSeriesNumber(
      capturedSlots,
      statHeader.frequencyQty,
      statHeader.seriesMode,
    );
    const used = new Set(usedSlotsInSeries(capturedSlots, open).map(String));
    let nextSlot = "";
    for (let i = 0; i < statHeader.frequencyQty; i++) {
    ...
    return fields.map((f) => {
      if (f.key === "line_slot") {
        return { ...f, disabledOptions: [...used], defaultValue: nextSlot };
      }
      if (f.key === "line_stamp") {
        return {
          ...f,
          ...
          required: false,
          stampConfig: {
            startDatetime: statHeader.startDatetime,
            frequencyType: statHeader.frequencyType,
            frequencyQty: statHeader.frequencyQty,
            series: open,
          },
        };
      }
  ...
  }, [fields, capturedSlots, panel.recordTypeApiName, viewMode, statHeader]);
  const statCanAdd = useMemo(() => {
    if (panel.recordTypeApiName !== "statistics" || viewMode === "instances") {
      return true;
    }
    if (!statHeader || statHeader.seriesMode === "dynamic") return true;
    const open = openSeriesNumber(
      capturedSlots,
      statHeader.frequencyQty,
      statHeader.seriesMode,
    );
    return usedSlotsInSeries(capturedSlots, open).length < statHeader.frequencyQty;
  }, [panel.recordTypeApiName, viewMode, statHeader, capturedSlots]);
  ...
  const toCells = useCallback(
    (inst: { id: string; name?: string; status?: string; data?: Record<string, string> }) => {
      ...
      if (cells.line_value) cells.line_value = formatStatLineValue(cells.line_value);
      return cells;
    },
    [catalogFields],
  );
  ...
  useEffect(() => {
    if (viewMode !== "instances" || panel.recordTypeApiName !== "page_element" || !selectedRowId) {
      return;
    }
    const controller = new AbortController();
    void fetch("/api/records/" + selectedRowId, {
      credentials: "include",
      signal: controller.signal,
    })
      .then((r) => (r.ok ? r.json() : { data: null }))
      .then((json) => {
        const inst = json.data as
          | { name?: string; data?: Record<string, string> }
          | null;
        if (!inst) return;
        const driver = (inst.data?.driver || "").trim();
        if (!driver) return;
        setPreview({
          kind: "canvas-driver",
          driver,
          label: inst.name || driver,
          note: inst.data?.preview || inst.data?.description,
        });
      })
      .catch(() => undefined);
    return () => controller.abort();
  }, [viewMode, panel.recordTypeApiName, selectedRowId, setPreview]);
  ...
  useEffect(() => {
    ...
    void fetch("/api/records/" + selectedRowId, {
      ...
      .then((json) => {
        const inst = json.data as
          | { id: string; name?: string; status?: string; data?: Record<string, string> }
          | null;
        if (!inst) return;
        setPreview({
          kind: "stat-graph",
  ...
  const validateRequired = (draft: Record<string, string>): string => {
    ...
    for (const f of catalogFields) {
      ...
      // Lines editor: header-placed required fields must not block capture.
      if (isHeaderLines && viewMode !== "instances" && role === "header") continue;
  ...
  const onDelete = async (id: string) => {
    ...
    try {
      if (isHeaderLines && panel.recordTypeApiName === "statistics") {
        // I5.6.35 S-4: statistic lines delete through the capture API.
        if (!headerRecordId) return false;
        const res = await fetch(
          "/api/statistics/" + headerRecordId + "/lines?lineId=" + encodeURIComponent(id),
          { method: "DELETE", credentials: "include" },
        );
        if (!res.ok) {
          const json = await res.json().catch(() => ({}));
          alert(json?.error?.message ?? "Failed to delete line");
          return false;
        }
        setRows((prev) => prev.filter((r) => r.id !== id));
        bumpLines();
  ...
  const onAdd = async (draft: Record<string, string>): Promise<boolean> => {
    ...
    if (viewMode === "instances") {
      ...
      if (panel.recordTypeApiName === "statistics") {
        const statParse = parseStatHeaderConfig(applyDerivedFrequencyStart(draft));
        if (!statParse.ok) {
          alert(statParse.errors.join(" "));
          return false;
        }
      }
      try {
        ...
        const data: Record<string, string> = applyDerivedFrequencyStart({ ...draft });
        delete data.Name;
        delete data.Status;
    ...
    try {
      ...
      delete data.Status;
      if (isHeaderLines && panel.recordTypeApiName === "statistics") {
        // I5.6.35 S-4: statistic lines are CAPTURED through the capture API
        // (same validation as monitoring scripts - scale bounds, frequency
        // window, series capacity). The generic record-lines PATCH path would
        // bypass the shared rules.
        if (!statCanAdd) {
          alert("Single Series is full — one series of points only.");
          return false;
        }
        if (!headerRecordId) {
        ...
        const slot = Number(data.line_slot);
        const value = Number(data.line_value);
        if (!Number.isInteger(slot) || !Number.isFinite(value)) {
          alert(
            frequencyTypeLabel(statHeader?.frequencyType) +
              " and Value are required.",
          );
          return false;
        }
        const open = statHeader
          ? openSeriesNumber(
              capturedSlots,
              statHeader.frequencyQty,
              statHeader.seriesMode,
          ...
          : 1;
        if (usedSlotsInSeries(capturedSlots, open).includes(slot)) {
          const labels = statHeader
            ? frequencySlotLabels(statHeader, statHeader.frequencyQty)
            : [];
          alert(
            (labels[slot] ?? String(slot)) +
              " is already captured. Choose a frequency value that is still open.",
          );
          return false;
        }
        const res = await fetch("/api/statistics/" + headerRecordId + "/lines", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify({ slot, value, source: "ui" }),
        });
        const json = await res.json().catch(() => ({}));
        if (!res.ok) {
          alert(json?.error?.message ?? "Failed to capture line");
          return false;
        }
        const line = json.data as {
          id: string;
          slot: number;
          value: string;
          series?: number;
        ...
        const nextCells = {
          line_slot: String(line.slot),
          line_value: formatStatLineValue(line.value),
          line_stamp: line.stamp ?? "",
          series: String(line.series ?? 1),
        };
        setRows((prev) => {
          const idx = prev.findIndex((r) => r.id === line.id);
          if (idx >= 0) {
            return prev.map((r) =>
              r.id === line.id ? { id: line.id, cells: nextCells } : r,
            );
          }
          return [...prev, { id: line.id, cells: nextCells }];
        });
        bumpLines();
        return true;
      }
  ...
  const onUpdate = async (id: string, draft: Record<string, string>): Promise<boolean> => {
    ...
    try {
      ...
      if (isHeaderLines && panel.recordTypeApiName === "statistics") {
        if (!headerRecordId) return false;
        const slot = Number(data.line_slot);
        const rawValue = String(data.line_value ?? "").trim();
        const value = rawValue === "" ? null : Number(rawValue);
        if (!Number.isInteger(slot) || (value !== null && !Number.isFinite(value))) {
          alert(
            frequencyTypeLabel(statHeader?.frequencyType) +
              " and Value are required.",
          );
        ...
        const res = await fetch("/api/statistics/" + headerRecordId + "/lines", {
          ...
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify({ lineId: id, slot, value, source: "ui" }),
        });
        const json = await res.json().catch(() => ({}));
        if (!res.ok) {
          alert(json?.error?.message ?? "Failed to update line");
          return false;
        }
        const line = json.data as {
          id: string;
        ...
        <EntityListing
          ...
          formatCell={
            ...
            statHeader
              ? (_row, key, raw) => {
                  if (key === "line_slot") {
                    const labels = frequencySlotLabels(
                      statHeader,
                      statHeader.frequencyQty,
                    );
                    const i = Number(raw);
                    return Number.isInteger(i) && labels[i] != null ? labels[i] : raw;
                  }
                  if (key === "line_value") return formatStatLineValue(raw);
                  if (key === "line_stamp") {
                    const slot = Number(_row.cells.line_slot);
                    const series = Number(_row.cells.series || 1) || 1;
                    const start = parseStartDatetime(statHeader.startDatetime);
                    if (start && Number.isInteger(slot)) {
                      return formatStamp(
                        stampForSlot(
                          start,
                          statHeader.frequencyType,
                          series,
                          slot,
                          statHeader.frequencyQty,
                        ),
                    ...
                    const parsed = parseStartDatetime(raw);
                    return parsed ? formatStamp(parsed) : raw;
                  }
                  return raw;
                }
              : undefined
          ...
          sortCell={
            ...
            statHeader
              ? (row, key, raw) => {
                  if (key === "line_stamp") {
                    const slot = Number(row.cells.line_slot);
                    const series = Number(row.cells.series || 1) || 1;
                    const start = parseStartDatetime(statHeader.startDatetime);
                    if (start && Number.isInteger(slot)) {
                      return stampForSlot(
                        start,
          ...
          expandedRowId={viewMode === "instances" ? expandedRowId : undefined}
          onExpandedRowChange={
            viewMode === "instances" ? onExpandedRowChange : undefined
          }
          renderExpandedRow={
            viewMode === "instances" && renderExpandedRow
              ? (row) => renderExpandedRow(row.id)
              : undefined
          }
          expandedEditing={viewMode === "instances" ? expandedEditing : undefined}
          onRowEdit={
            viewMode === "instances" && onRowEdit
              ? (row) => onRowEdit(row.id)
              : undefined
          }
...
function FormPanel({
  ...
  /** Open the expanded record already in edit mode (listing Edit). */
  startEditing?: boolean;
  onEditingChange?: (editing: boolean) => void;
  /** When false, do not overwrite Spatial Twin (another header is the graph target). */
  publishTwin?: boolean;
}) {
  type CatalogField = { api_name: string; label: string; data_type: string; value_set_api_name?: string | null; active?: boolean; is_required?: boolean; is_secret?: boolean; zone_role?: "header" | "list" | null; lookup_object_api_name?: string | null };
  const isDynamic = !!panel.recordTypeApiName;
  const objectApiName = panel.objectApiName || panel.recordTypeApiName || "";
  const [catalogFields, setCatalogFields] = useState<CatalogField[]>([]);
  const runtime = useSavedRuntimeLayouts(
  ...
  const { setPreview, bumpLines } = useTwinSlot();

  useEffect(() => {
    if (!session?.userId) return;
    setValues((prev) => {
      const next = { ...prev };
      if (!next.created_by) next.created_by = session.userId;
      if (editing || !instanceId) next.last_modified_by = session.userId;
      return next;
    });
  }, [session, editing, instanceId]);

  useEffect(() => {
    if (panel.recordTypeApiName !== "statistics") return;
      ...
      void fetch("/api/records/" + detailRecordId, { credentials: "include", signal: controller.signal })
        ...
        .then((json) => {
          ...
          if (inst) {
            ...
            const start = parseStartDatetime(draft.start_datetime);
            if (start) draft.start_datetime = toDatetimeLocalValue(start);
            setValues(draft);
  ...
  const sections =
    isDynamic &&
    objectApiName &&
    layoutForMode.length &&
    layoutForMode.some((sec) => sec.fields.length > 0)
      ? layoutForMode
          .map((sec) => ({
            ...sec,
            fields: filterByZoneRole(sec.fields, "header").filter((f) =>
              objectApiName === "statistics"
                ? f.blank === true || isLiveStatField(f.key)
                : true,
            ),
          }))
  ...
  const onSave = async () => {
    ...
    // I5.6.35 S-4: statistic headers validate through the shared model (the
    // ONE layer the capture API also uses) so a malformed header can never be
    // saved from the UI.
    if (panel.recordTypeApiName === "statistics") {
      const statParse = parseStatHeaderConfig(applyDerivedFrequencyStart(values));
      if (!statParse.ok) {
        setSaveError(statParse.errors.join(" "));
        setSaveStatus("");
        return;
      }
    }
    ...
    setSaveStatus("");
    try {
      const data: Record<string, string> = applyDerivedFrequencyStart({ ...values });
      ...
      if (panel.recordTypeApiName === "statistics") bumpLines();
    } catch (e: unknown) {
            ...
            <LayoutDrivenForm
              ...
              onChange={(key, value) =>
                setValues((d) => {
                  ...
                  return key === "frequency_type" || key === "start_datetime"
...
function TabPanel({
  ...
  const isOrganizationsSelf = tab.id === "org-configuration" && panel.id === "org-organizations";
  const isOrgConfiguration = tab.id === "org-configuration" && panel.id === "org-configuration";
    ...
    <div className="space-y-3">
      ...
      {isOrgConfiguration ? (
        <PrimaryOrgPanel accent={accent}>
          {/* #298: Head/Deputy folded inside the Configuration card (Stephen 2026-09-13). */}
          <DivisionConfigPanel
        ...
        <div className="space-y-4">
          ...
          />
          {panel.recordTypeApiName === "statistics" ? (
            <StatGraphPanel headerId={headerRecordId} />
          ) : null}
          <ListingPanel
            panel={panel}
            accent={accent}
...
export function ZoneConfigView({
  config,
  chrome = "full",
  showTwin,
}: {
  ...
  return (
    <TwinSlotProvider>
      <ZoneConfigViewBody config={config} chrome={chrome} showTwin={showTwin} />
    </TwinSlotProvider>
  );
}

function ZoneConfigViewBody({
  config,
  chrome = "full",
  showTwin,
}: {
  ...
  const focusedNodeId = useMemo(
    () => twinFocusNodeId(config.id, active, childId),
    [config.id, active, childId],
  );
  ...
  const handleNodeClick = useCallback(
    (node: { id: string }) => {
      const resolved = resolveNodeToTab(node.id, { ...config, tabs });
      if (!resolved) return;
  ...
  /** Custom has no 3D hub. Statistics still gets the stat-graph twin on its home listing. */
  const customStatsTwin =
    active === "custom" &&
    (childId === "rt-environment_stat" ||
      childId === "statistics" ||
      childId === "stats");
  const showHubTwin =
    showTwin ?? (Boolean(hubZone) && (active !== "custom" || customStatsTwin));
  ...
  return (
    <div className="space-y-3">
      ...
      {chrome === "full" && tab && subTabs.length > 0 ? (
        <SubTabBar
          items={subTabs}
      ...
      <div role="tabpanel" className="relative flex min-h-[max(640px,calc(100dvh-13rem))] flex-col gap-4 lg:flex-row lg:items-start">
        ...
        {showHubTwin && hubZone ? (
          <div className="sticky top-3 z-20 hidden lg:flex shrink-0 items-start self-start" style={{ paddingLeft: 10 }}>
            <button
              type="button"
              onClick={() => setTwinOpenPersist(!twinOpen)}
              className="hidden lg:flex z-10 mt-2 h-20 w-5 shrink-0 items-center justify-center rounded-md border border-border bg-card text-muted-foreground shadow-sm transition-colors hover:bg-muted hover:text-foreground"
            ...
            {twinOpen ? (
              <div data-hydrated={twinHydrated ? "1" : "0"}>
                <SpatialTwinPane
                  zoneId={config.id}
                  hubZone={hubZone}
        ...
        {/* Mobile (Stephen 23:22): the twin floats in from the bottom edge —
            right-side handle pill, drag/tap to pull up or push away, remembered. */}
        {showHubTwin && hubZone ? (
          <FloatTwinDock
            zoneId={config.id}
            hubZone={hubZone}
