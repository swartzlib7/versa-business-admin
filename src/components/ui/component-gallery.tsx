    ...
    components: [
      {
        ...
        example: (
          <Card size="hug" className="w-full max-w-sm">
            <CardHeader>
    ...
    <div className="space-y-6">
      ...
      {currentSection && (
        <div className="space-y-4">
          ...
          {/* Component Demos — compact gallery (Stephen 2026-09-14 03:06):
              six per row at desktop, roughly half the old box size/height, so
              a whole family (Buttons, Inputs, Badges, Cards, Separators) fits
              on one screen. `size="hug"` drops the 640px card floor. */}
          <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
            {currentSection.components.map((component) => (
              <Card key={component.name} size="hug" className="min-w-0">
                <CardHeader className="gap-0.5 px-3">
                  <CardTitle className="text-sm">{component.name}</CardTitle>
                  <CardDescription className="text-xs leading-snug">{component.description}</CardDescription>
                </CardHeader>
                <CardContent className="space-y-2 px-3">
                  {/* Live Example */}
                  <div className="flex min-h-[56px] items-center justify-center overflow-hidden rounded-md border border-border bg-muted/30 p-2 [&_[data-slot=card]]:min-h-0">
                    {component.example}
                  </div>
                  ...
                  {showCode[component.name] && (
                    <pre className="overflow-x-auto rounded-md bg-muted p-2 text-[11px] leading-snug">
