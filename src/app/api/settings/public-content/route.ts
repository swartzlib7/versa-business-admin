...
async function payload() {
  ...
  const base = normalizePublicContent(settings);
  const cycle_steps = await loadCycleSteps(settings);
  return { ...base, cycle_steps, cycle_enabled: cycle_steps.length > 0 || base.cycle_enabled };
}
